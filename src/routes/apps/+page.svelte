...
<svelte:head>
	...
	<meta
		name="description"
		content="Discover 30+ privacy-focused applications. Every app verified for security and privacy."
	/>
</svelte:head>
...
<div class="apps-page">
	...
	<section class="hero container py-12 text-center relative z-10">
		<h1 class="text-4xl md:text-5xl font-black mb-4 text-gradient">Privacy-First Applications</h1>
		<p class="text-lg text-text-secondary max-w-2xl mx-auto">
			Every app in the VU Suite is built with privacy as the foundation. Browse our collection of
			30+ zero-knowledge applications.
		</p>
	</section>
			...
			{#each categories as category}
				<a
					...
					class:active={category === 'All'
						...
						: $page.url.searchParams.get('category')?.toLowerCase() === category.toLowerCase()}
				>
					{category}
				</a>
			{/each}
	...
	<section class="apps-grid-section container mb-16 relative z-10">
		<div class="app-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
			{#each filteredApps as app}
				<a
					href="/apps/{app.id}"
					class="app-card glass-card p-6 hover:-translate-y-1 transition-all"
				>
					...
					<div
						class="app-card-description text-sm text-text-secondary leading-relaxed mb-4 line-clamp-3"
					>
						{app.description}
					</div>
