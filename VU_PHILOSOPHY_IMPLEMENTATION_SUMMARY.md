...
**Everywhere:**
```svelte
<h2>{user.username}</h2>       <!-- @privacy_champion -->
<p>{user.email}</p>             <!-- privacy_champion@vumail.app -->
<div>Level {user.privacyLevel}</div>  <!-- Level 0 -->
```

**Never:**
```svelte
<h2>{user.firstName} {user.lastName}</h2>  <!-- ❌ NEVER -->
<p>{user.phone}</p>                        <!-- ❌ NEVER -->
<p>{user.address}</p>                      <!-- ❌ NEVER -->
```
